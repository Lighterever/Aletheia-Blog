...
│   ├── build.js            Unified build
│   ├── editor-server.js    Local Markdown editor server (port 3333)
│   ├── serve.py            Python dev server (port 3000)
│   └── encrypt.js          AES encryption utility
│
├── editor/                 Editor front-end page
├── doc/                    Docs (quick ref, rendering guide)
│
...
### 🖊️ Local Markdown Editor

A built-in editor for local use only — readers will never see it:

```bash
npm run editor
```

Auto-opens browser → write markdown on the left, live preview on the right. One-click format toolbar, smart Enter auto-continues lists, `Ctrl+S` to save, "Build" button to publish.

> **Why it's safe online**: The editor is a local `node scripts/editor-server.js` process. Vercel only runs `npm run build` to generate static files — readers can never access the editor.

### Supported Markdown Formats

| Document | Description |
|----------|-------------|
| [Quick Reference](doc/quick-reference.md) | All formats at a glance |
| [Rendering Guide](doc/markdown-support.md) | Full format list, pipeline, CSS system |
